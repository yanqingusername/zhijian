        ...
        exit(path, {
          ...
          while (index >= 0) {
            ...
            if (_utils().t.isExportAllDeclaration(d) && d.source) {
              var _opts$onTransformDeps2, _opts$exportAllMember;
              ...
              if (isMatch && ((_opts$exportAllMember = opts.exportAllMembers) === null || _opts$exportAllMember === void 0 ? void 0 : _opts$exportAllMember[d.source.value])) {
                const id = _utils().t.identifier('__all_exports');
                ...
                path.node.body[index] = _utils().t.exportNamedDeclaration(_utils().t.variableDeclaration('const', [_utils().t.variableDeclarator(_utils().t.objectPattern(opts.exportAllMembers[d.source.value].map(m => _utils().t.objectProperty(_utils().t.identifier(m), _utils().t.identifier(m)))), _utils().t.identifier('__all_exports'))]));
              }
            } // export { bar } from 'foo';
