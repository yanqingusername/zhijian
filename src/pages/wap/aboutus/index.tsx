...
import WapPartnerSwiper from '../components/partnerSwiper';
import WapPositions from '../components/positionList';
import './index.less';

interface WapAboutUsInitialProps extends IGetInitialProps {
    logos?: string[];
    miniIcon?: string;
    mpIcon?: string;
    focus?: string;
...
const WapAboutUs = (props: WapAboutUsInitialProps) => {
    const { logos, miniIcon, mpIcon, focus, partners, positions } = props;
    ...
    return (
        <div className="wap-pageContainer">
            <WapHeader
                ...
                page="aboutus"
            />
            <div className="aboutus-focus" style={{ backgroundImage: `url(${focus})` }}>
                <h1>ABOUT US</h1>
                <h3>关于我们</h3>
            </div>
            <div className="mainContent">
                ...
                <div id="culture" className="aboutus-block">
                ...
                <div id="business" className="aboutus-block">
                ...
                <div id="joinus" className="aboutus-block">
...
WapAboutUs.getInitialProps = (async () => {
    ...
    const focus = nbp.request.mediaUrl((await nbp.api.getAdverts({ code: 'ABOUTUS-FOCUS', provider: 'M', single: true })).space.source);
    const partners = ((await nbp.api.getAdverts({ code: 'LINKS', provider: 'M', single: false })).items.map(i => { return { id: i.id, path: i.path, source: i.source } }));
    const positions = ((await nbp.api.getArticles({ category: '12', maxResultCount: 100 })).items?.map((i) => {
        return { id: i.id, title: i.title }
    }));
    return Promise.resolve({ logos, miniIcon, mpIcon, focus, partners, positions });
}) as IGetInitialProps;

export default WapAboutUs;